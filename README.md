# 乐心蓝牙设备小程序插件接入指南
demo地址:https://github.com/leshiguang/LSOldBluetoothPluginDemo.git
...
#### 5.13 获取WIFI列表
```javascript
let getWifi = new lsPlugin.SettingProfile.ReqScan();

//push to device
lsPlugin.pushSettings(deviceMac, getWifi, onSettingListener);
```


#### 5.14 连接WIFI
```javascript
  let connectWifi = new lsPlugin.SettingProfile.ConnectWifi(bssid, password);

  //push to device
  lsPlugin.pushSettings(deviceMac, connectWifi, onSettingListener);
  
```
* 注：WIFI列表和配网结果将在 `_syncCallback` 的 `onDataChanged()` 方法返回
...
#### 6.10 扫描到WIFI信息  dataType=（0x1000）
...
#### 6.10 扫描结束，返回WIFI列表  dataType=（0x1001）
...
var _wifiInfo = {
  ...
  ssid:"",
  bssid:"",
  status:0,           //return success (0) or failed reason code (1)
  ip:"",
}
```

#### 6.12 WIFI配网状态回包  dataType=（0x1002）
...
```
注意：在使用插件的时候，有使用其他蓝牙设备，导致sdk内对wx.onBLEConnectionStateChange wx.onBLECharacteristicValueChange 注册事件失效的情况，
那么需要调用reinit接口，重新初始化
```javascript
const lsPlugin = requirePlugin("LSDevicePlugin")
...
#### 5.13 获取WIFI列表
```javascript
let getWifi = lsPlugin.SettingProfile.ReqScan();
...
#### 5.14 连接WIFI
```javascript
  let connectWifi = lsPlugin.SettingProfile.ConnectWifi(ssid, password);
...
var _bpData = {
    ...
    flag: 0x00,              //标志位
    uint: '',                //单位,mmHg、Kpa
    systolic: 0,             //高压数据，以 mmHg 上传，根据 unit flag 的定义进行显示
    diastolic: 0,            //低压数据，以 mmHg 上传，根据 unit flag 的定义进行显示
    meanPressure: 0,         //平均值数据
    pulseRate: 0,            //脉搏数据
    utc: 0,                  //utc
    userNumber: null,        //用户编号
    timeZone: null,          //时区
