...
Page({
  ...
  connectDevice: function() {
    ...
    this.updateStatusMessage('NCP Connecting...');
    ...
    let device = {deviceName: this.data.deviceName, deviceMac: mac};
    let self = this;
    let callback = {
      onConnectionStateChanged: function( status) {
        self.appendLogText(`设备连接状态发现改变`);
        self.setData({statusMsg: TEXTS[status]});
        self.setData({connectState: device.status});
      },
      onDataChanged: function(deviceMac, dataType, data, dataStr) {
        if (data.image) {
          console.log('回调图片', data.image);
          console.log('文1件数据', data.fileData);
          
        }
        if (dataType === 1002) {
        }
        console.log('ui data changed', data);
        self.appendLogText('数据' + JSON.stringify(data));
      },
    };
    connectNcpDevice(device, callback);
    this.updateStatusMessage('Connecting...' + JSON.stringify(device));
