...
Page({
  ...
  onLoad: function(options) {
    ...
    addEventListener('connectionStateChange', (device) => {
      if (device) {
        self.appendLogText(`设备状态改变:${TEXTS[device.status]}${JSON.stringify(device)}`);
    ...
    //注册运动事件回调监听
    addEventListener('sportEvent',(deviceMac, sportType) => {
      self.appendLogText('sportEvent： ' ,deviceMac, sportType);
    });
